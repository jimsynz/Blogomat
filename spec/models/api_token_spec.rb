...
describe ApiToken do
  ...
  subject { ApiToken.new(existing_token, fake_redis) }
  ...
  def set_last_user_to(user)
    ApiToken.new(existing_token, fake_redis).user = user
  end
  ...
  context "When refreshing an existing (not expired) token" do
    let(:existing_token) { ApiToken.new(nil,fake_redis).token }
  ...
  describe "#delete" do
    let(:token) { ApiToken.new(nil, fake_redis) }
    before { token }
